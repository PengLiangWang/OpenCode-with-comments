...
{
    memory_block *pfree_mem_addr;       //节点内容为 memory_block 结构体, 组成一个chunk,    多个chunk组成chunk_pool
    memory_chunk *pre;
    memory_chunk *next;
}memory_chunk;
...
typedef struct MEMORYPOOL
{
    ...
    size_t           mem_used_size;         //记录内存池中已经分配给用户的大小   
    size_t           mem_map_pool_count;   //记录链表单元缓冲池中剩余的单元的个数, 个数为0时不能分配单元给 pfree_mem_chunk
    size_t           free_mem_chunk_count;   //记录 pfree_mem_chunk 链表中的单元个数
